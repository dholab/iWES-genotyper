...
if len(in_dirs) > 0:
    for depth_dir in in_dirs:
        ...
        file_list = [
            ...
            for x in file_list
            if not x.startswith("._") and x.endswith(".csv")
        ]
        df = pd.DataFrame()
        print(len(file_list))
        i = 0
        ...
        df["MAX_POSITION"] = df.groupby(["SAMPLE_NUM", "ALLELE"])["POSITION"].transform(
            "max"
        )
        ...
        df_summary_i = (
            df.groupby(
                ["SAMPLE_NUM", "ALLELE", "HAPLOTYPE_GROUP", "unique_maps_per_allele"]
            )
            .agg({"DEPTH": ["min", "median"]})
            .reset_index()
        )

        df_summary_i.columns = ["_".join(col) for col in df_summary_i.columns.values]
        df_summary_i.columns = [
            col[:-1] if col.endswith("_") else col for col in df_summary_i.columns
        ]
        # df_summary.columns = [col[:-7] if col.endswith('_median') else col for col in df_summary.columns]

        df_summary_i["SAMPLE_MEDIAN_DEPTH"] = df_summary_i.groupby(["SAMPLE_NUM"])[
            "DEPTH_median"
        ].transform("median")
        df_summary_i["FILE_NUM"] = file_num
        df_summary_i.to_csv(
            os.path.join(out_dir, f"all_depth_summary_{file_num}.csv"), index=False
        )
...
print(
    "summary exported to {0}".format(os.path.join(out_dir, "summary_correlation.csv"))
)
df_summary["MEDIAN_RATIO"] = (
    df_summary["DEPTH_median"] / df_summary["SAMPLE_MEDIAN_DEPTH"]
)


# remove outlier ratios
df_summary_f = df_summary[
    (df_summary["MEDIAN_RATIO"] < 1.3) & (df_summary["MEDIAN_RATIO"] > 0.7)
]
predict_dict = {}
...
for haplotype_group in haplotype_group_list:
    ...
    if len(df_summary_h) < 1:
        continue
    lin_regression = LinearRegression().fit(
        df_summary_h["SAMPLE_MEDIAN_DEPTH"].values.reshape(-1, 1),
        df_summary_h["DEPTH_min"].values.reshape(-1, 1),
    )
    # lin_regression

    model_line = lin_regression.predict(
        df_summary_h["SAMPLE_MEDIAN_DEPTH"].values.reshape(-1, 1)
    )
    # model_line
    plt.scatter(
        df_summary_h["SAMPLE_MEDIAN_DEPTH"].values.reshape(-1, 1),
        df_summary_h["DEPTH_min"],
    )
    ...
    plt.savefig(
        os.path.join(out_dir, "{0}_summary_correlation.png".format(haplotype_group))
    )
    plt.clf()
    sum_errs = np.sum(
        (df_summary_h["DEPTH_min"].values.reshape(-1, 1) - model_line) ** 2
    )
    stdev = np.sqrt(
        1 / (len(df_summary_h["DEPTH_min"].values.reshape(-1, 1)) - 2) * sum_errs
    )
    predict_dict[haplotype_group] = [
        lin_regression.coef_[0][0].round(3),
        lin_regression.intercept_[0].round(3),
        stdev.round(3),
    ]

    print(
        "CorrelationPlot exported to {0}".format(
            os.path.join(out_dir, "{0}_summary_correlation.png".format(haplotype_group))
        )
    )
# Data to be written
...
print(
    "Linear correlation coeff's (slope, int, stdev) exported to {0}".format(
        os.path.join(out_dir, "confidence_coeff.json")
    )
)
