...
def str2ram(v):
    ...
    if v.lower().endswith in ("kb"):
        ...
        raise ArgumentTypeError(
            "Ram value expected (int in mb, or ends with KB, MB, GB, or TB."
        )
...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--out_dir", type=str, help="directory to output the results", required=True
    )
    parser.add_argument(
        "--bam_dir",
        type=str,
    ...
    parser.add_argument(
        "--project_name", type=str, help="Project name for file prefix", required=True
    )
    parser.add_argument(
        "--bait_fasta",
        type=str,
...
def filter_count_map_bam(df_bam_m, allele_list):
    ...
    df_bam_m["num_read_maps"] = df_bam_m.groupby(["NAME"])["NAME_COUNTS"].transform(
        "sum"
    )
    df_bam_m["unique_maps_per_allele"] = df_bam_m.groupby(["ALLELE"])[
        "num_read_maps"
    ].transform("min")
...
def process_df_bam(df_bam, ipd_length_dict, sample_i, unpaired_edge_threshold):
    ...
    df_bam["DISTANCE_TO_END"] = [
        ipd_length_dict[y] - x for x, y in zip(df_bam["END"], df_bam["ALLELE"])
    ]
    df_bam["INCLUDE_UNPAIRED"] = [
        1
        if ((x < unpaired_edge_threshold) and z)
        or ((y < unpaired_edge_threshold) and not (z))
        else 0
        for x, y, z in zip(
            df_bam["START"], df_bam["DISTANCE_TO_END"], df_bam["REVERSED_MAPPING"]
        )
    ]
    ...
    df_bam["unique_maps_per_allele"] = df_bam.groupby(["ALLELE"])[
        "num_read_maps"
    ].transform("min")
    return df_bam
...
def get_gap_allele_list(df_bam_m, max_gap=70):
    ...
    df_bam_m_dedup = df_bam_m[
        ["NAME", "REVERSED_MAPPING", "START", "END", "PAIRED", "ALLELE", "SAMPLE_NUM"]
    ].drop_duplicates()
    df_bam_m_dedup["START_ALLELE_MIN"] = df_bam_m_dedup.groupby(
        ["ALLELE", "SAMPLE_NUM"]
    )["START"].transform("min")
    df_bam_m_dedup["END_ALLELE_MAX"] = df_bam_m_dedup.groupby(["ALLELE", "SAMPLE_NUM"])[
        "END"
    ].transform("max")
    ...
    df_bam_paired["START_MIN"] = df_bam_paired.groupby(
        ["ALLELE", "SAMPLE_NUM", "NAME"]
    )["START"].transform("min")
    df_bam_paired["END_MAX"] = df_bam_paired.groupby(["ALLELE", "SAMPLE_NUM", "NAME"])[
        "END"
    ].transform("max")
    ...
    df_bam_overlap["START"] = [
        y if x < y else x
        for x, y in zip(df_bam_overlap["START_MIN"], df_bam_overlap["START_ALLELE_MIN"])
    ]
    df_bam_overlap["END"] = [
        y if x > y else x
        for x, y in zip(df_bam_overlap["END_MAX"], df_bam_overlap["END_ALLELE_MAX"])
    ]
    ...
    df_gap_summary = (
        df_gap_summary.groupby(["ALLELE", "SAMPLE_NUM", "MAX_GAP"])["POSITION"]
        .min()
        .reset_index()
    )
...
def make_range_from_fasta_lengths_dict(ipd_length_dict, allele_list):
    ...
    for allele_i in allele_list:
        df_depth_range_i = pd.DataFrame(
            {"ALLELE": allele_i, "POSITION": list(range(0, ipd_length_dict[allele_i]))}
        )
        df_depth_range = pd.concat(
            [df_depth_range, df_depth_range_i], ignore_index=True
        )
    return df_depth_range


def get_depth_and_filter(
    bam_filepath,
    ...
    cmd = ["samtools", "depth", "-a", "-o", "-", bam_filepath]
    a = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    b = StringIO(a.communicate()[0].decode("utf-8"))

    df_depth = pd.read_csv(
        b, sep="\t", header=None, names=["ALLELE", "POSITION", "DEPTH"]
    )
    if output_depth_all:
        os.makedirs(os.path.join(out_dir, "depth_all"), exist_ok=True)
        depth_dir = os.path.join(
            out_dir, "depth_all", "{0}.depth_all{1}.csv".format(sample_i, db_ext_i)
        )
    ...
    if len(df_range_mask) > 0:
        ...
        ups_dict = {}
        for allele_i in allele_list:
            lows_dict[allele_i] = np.array(
                list(df_range_mask[allele_i == df_range_mask["ALLELE"]]["START"])
            )
            ups_dict[allele_i] = np.array(
                list(df_range_mask[allele_i == df_range_mask["ALLELE"]]["END"])
            )
        ...
        df_depth["EXCLUDE"] = [
            in_range(x, lows_dict[y], ups_dict[y])
            for x, y in zip(df_depth["POSITION"], df_depth["ALLELE"])
        ]
    else:
...
def depth_worker(allele_chunk):
    ...
    df_bam_m_dedup_i["POSITION"] = [
        list(range(x, y))
        for x, y in zip(df_bam_m_dedup_i["START"], df_bam_m_dedup_i["END"])
    ]
    ...
    gc.collect()
    df_depth_i = (
        df_depth_i.groupby(["ALLELE", "POSITION", "unique_maps_per_allele"])
        .agg({"START": "count", "num_read_maps": "sum"})
        .rename(columns={"START": "DEPTH"})
        .reset_index()
    )
    df_depth_i["DEPTH_ADJ"] = (
        df_depth_i["DEPTH"] * df_depth_i["DEPTH"] / df_depth_i["num_read_maps"]
    )
    ...
    df_depth_i = df_depth_i.merge(
        df_depth_range_i, on=["ALLELE", "POSITION"], how="outer"
    )
...
def get_depth(df_bam_m, ipd_length_dict, threads=1):
    ...
    df_bam_m_dedup = df_bam_m[
        ["ALLELE", "NAME", "START", "END", "unique_maps_per_allele", "num_read_maps"]
    ].drop_duplicates()
    ...
    for i in range(start, end, step):
        ...
        df_depth_range_i = df_depth_range[
            df_depth_range["ALLELE"].isin(allele_list[i : i + step])
        ]
        df_bam_m_dedup_i = df_bam_m_dedup[
            df_bam_m_dedup["ALLELE"].isin(allele_list[i : i + step])
        ]
        allele_chunk_list.append(
            {"df_depth_range": df_depth_range_i, "df_bam_dedup": df_bam_m_dedup_i}
        )
    ...
    df_depth["unique_maps_per_allele"] = df_depth.groupby(["ALLELE"])[
        "unique_maps_per_allele"
    ].transform("max")
    return df_depth


def get_normalized_median_by_allele_sample(
    df_depth, median_groups, ipd_length_dict=[], depth_threshold=1
):
    if len(ipd_length_dict) > 0:
        df_depth_copy = df_depth.copy()
        df_depth_copy = df_depth_copy[df_depth_copy["DEPTH"] >= depth_threshold]
        df_summary = (
            df_depth_copy.groupby(["ALLELE", "SAMPLE_NUM", "HAPLOTYPE_GROUP"])
            .agg(
                {
        ...
        df_summary.columns = [
            col[:-1] if col.endswith("_") else col for col in df_summary.columns
        ]
        df_summary.columns = [
            col[:-7] if col.endswith("_median") else col for col in df_summary.columns
        ]
        df_summary["Depth_Threshold_From_End_Position"] = [
            ipd_length_dict[y] - x - 1
            for x, y in zip(df_summary["POSITION_max"], df_summary["ALLELE"])
        ]
        ...
        df_summary = (
            df_depth.groupby(["ALLELE", "SAMPLE_NUM", "HAPLOTYPE_GROUP"])
            .agg({"unique_maps_per_allele": "median", "DEPTH_ADJ": "median"})
            .reset_index()
        )
    if len(median_groups) > 0:
        df_summary["HAPLOTYPE_GROUP_temp"] = [
            x.split("-")[1] if len(x.split("-")) > 1 else x
            for x in df_summary["HAPLOTYPE_GROUP"]
        ]
        df_summary_median = df_summary[
            df_summary["HAPLOTYPE_GROUP_temp"].isin(median_groups)
        ]
...
def calc_flag(
    read_mapped, mate_unmapped=True, reverse=True, first_in_pair=1, read_paired=1
):
...
def makebam(df_bam_in, df_mapped, ipd_length_dict, out_sam):
    ...
    df_mapped_dedup = df_mapped[
        ["NAME", "REVERSED_MAPPING", "START", "END", "PAIRED", "ALLELE"]
    ].drop_duplicates()
    ...
    df_unmapped["FLAG"] = [
        calc_flag(
            read_mapped=w, mate_unmapped=x, reverse=y, first_in_pair=z, read_paired=1
        )
        for w, x, y, z in zip(
            df_unmapped["READ_MAPPED"],
    ...
    df_unpaired["CIGAR"] = [
        cigar_string(start, end, read_len)
        for start, end, read_len in zip(
            df_unpaired["START"], df_unpaired["END"], df_unpaired["READ_LEN"]
        )
    ]
    ...
    df_paired["CIGAR"] = [
        cigar_string(start, end, read_len)
        for start, end, read_len in zip(
            df_paired["START"], df_paired["END"], df_paired["READ_LEN"]
        )
    ]
    df_paired["FIRST_IN_PAIR"] = df_paired.groupby(["ALLELE", "NAME"])["START"].rank(
        method="first"
    )
    df_paired["READ_MAPPED"] = True
    df_paired["MATE_UNMAPPED"] = False
    df_paired["START_1"] = df_paired.groupby(["ALLELE", "NAME"])["START"].transform(
        "min"
    )
    ...
    with open(out_sam, mode="w") as fout:
        ...
        df_header["SAMLINE"] = (
            "@SQ\tSN:" + df_header["allele"] + "\tLN:" + df_header["length"] + "\n"
        )
    ...
    sort_bam_cmd = ["samtools", "sort", "-o", out_bam, "{0}.bam".format(out_bam)]
...
def expand_bam_hash_worker(chunk, df_bam_in):
    ...
    df_exp_i["START_NEW"] = (
        df_exp_i["START_2_x"] - df_exp_i["START_2_y"] + df_exp_i["START"]
    ).clip(0)
    df_exp_i["END_NEW"] = (
        df_exp_i["END_2_x"] - df_exp_i["START_2_y"] + df_exp_i["START"]
    )
    # Fast method for:    df_exp_i['START_NEW'] = [0 if x < 0 else x for x in  df_exp_i['START_NEW']]
    df_exp_i["END_NEW"] = df_exp_i["END_NEW"] - (
        df_exp_i["END_NEW"] - df_exp_i["REF_LEN_2"]
    ).clip(0)
    df_exp_i["END_NEW"] = df_exp_i["END_NEW"] - (
    ...
    df_exp_i = df_exp_i[
        ((df_exp_i["START_NEW"] == 0) & (df_exp_i["END_NEW"] == df_exp_i["REF_LEN"]))
        | (
    ...
    df_exp_i = df_exp_i[
        (df_exp_i["READ_LEN"] / 2) < (df_exp_i["END_NEW"] - df_exp_i["START_NEW"])
    ]
    df_exp_i.drop(
        columns=["START_2_y", "END_2_y", "REF_LEN_2", "START", "END", "READ_LEN"],
        inplace=True,
    )
    df_exp_i.rename(
        columns={
...
def expand_bam_hash(df_hash, hash_chunks_n, df_bam_in, threads=1):
    ...
    df_all.drop_duplicates(
        subset=["NAME", "ALLELE_2", "REVERSED_MAPPING", "ALLELE"],
        keep="first",
        inplace=True,
    )
...
bam_filelist = [
    os.path.join(bam_dir, x)
    for x in bam_filelist
    if x.endswith(".bam") and not x.startswith("._")
]
df_median_dict = {}
df_gap_dict = {}
# convert to - notation.
...
if os.path.isfile(ipd_ref_hash):
    ...
    df_length = pd.DataFrame(
        list(ipd_length_dict.items()), columns=["ALLELE", "REF_LEN"]
    )
...
for bam_file_i in bam_filelist:
    ...
    df_read_ct_i = pd.DataFrame(
        {"sample_read_ct": [segment_count], "gs_id": [sample_i]}
    )
    df_read_ct_all = pd.concat([df_read_ct_all, df_read_ct_i], ignore_index=True)
    if len(df_bam_in) < 1:
        print(
            "Bam file could not be converted to a dataframe (could be empty): {0}".format(
                sample_i
            )
        )
        continue
    ...
    print(time.time() - t2)
    # hash_chunks_n = 4
    df_bam_in_t = df_bam_in[
        ["NAME", "ALLELE_2", "START_2", "END_2", "REVERSED_MAPPING", "READ_LEN"]
    ]
    ...
    df_all["PAIRED"] = df_all.groupby(["NAME", "ALLELE"])["REVERSED_MAPPING"].transform(
        "count"
    )
    ...
    for db_ext_i in db_ext:
        ...
        if not ((len(df_bam_in) > 0) and (len(df_bam) > 0)):
            continue
        print(
            "Export bam file including alleles without full coverage: {0}".format(
                sample_i
            )
        )
        ...
        allele_list = get_depth_and_filter(
            bam_filepath=out_bam,
        ...
        print(
            "Filter for depth and make depth coverage using samtools", time.time() - t2
        )
        df_bam = filter_count_map_bam(df_bam, allele_list)
        print("filter_count_map_bam", time.time() - t2)
        if len(df_bam) < 1:
            print(
                "1st  Depth of coverage (DOC < 0) filter filtered out all alleles for sample: {0}".format(
                    sample_i
                )
            )
            continue
        ...
        df_gap_dict[db_ext_i] = pd.concat(
            [df_gap_dict[db_ext_i], df_gap], ignore_index=True
        )
        del df_gap
        os.makedirs(
            os.path.join(out_dir, "expanded_maps{0}".format(db_ext_i)), exist_ok=True
        )
        df_bam.to_csv(
            os.path.join(
                out_dir,
        ...
        df_depth.to_csv(
            os.path.join(
                ...
                "{0}_depth{1}.csv".format(sample_i, db_ext_i),
            ),
            index=False,
        )
        print("Make intermediate files", time.time() - t2)
        df_norm_median_i = get_normalized_median_by_allele_sample(
            df_depth,
            median_groups=["A1", "B"],
            ipd_length_dict=ipd_length_dict,
            depth_threshold=depth_threshold,
        ...
        del df_depth
        df_norm_median_i = df_norm_median_i.merge(
            df_gap_summary_i, on=["ALLELE", "SAMPLE_NUM"], how="inner"
        )
        del df_gap_summary_i
        os.makedirs(
            os.path.join(out_dir, "normalized_median{0}".format(db_ext_i)),
            exist_ok=True,
        )
        df_norm_median_i.to_csv(
            os.path.join(
                out_dir,
        ...
        if (len(df_bam_in) > 0) and (len(df_bam) > 0):
            ...
            makebam(
                ...
                out_sam=os.path.join(
                    out_dir,
                    "bam{0}".format(db_ext_i),
                    "{0}{1}.sam".format(sample_i, db_ext_i),
                ),
            )
        print("--- {0} complete for {1} ---".format(db_ext_i, sample_i))
        del df_bam

        df_median_dict[db_ext_i] = pd.concat(
            [df_median_dict[db_ext_i], df_norm_median_i], ignore_index=True
        )
...
for db_ext_i in db_ext:
    if len(df_median_dict[db_ext_i]) > 0:
        df_median_dict[db_ext_i]["DEPTH_ADJ"] = df_median_dict[db_ext_i][
            "DEPTH_ADJ"
        ].astype(int)
        df_median_dict[db_ext_i]["DB"] = re.sub(r"\W+", "", db_ext_i)
        df_norm_median_all = pd.concat(
            [df_norm_median_all, df_median_dict[db_ext_i]], ignore_index=True
        )
        df_gap_dict[db_ext_i]["DB"] = re.sub(r"\W+", "", db_ext_i)
        df_gap_all = pd.concat([df_gap_all, df_gap_dict[db_ext_i]], ignore_index=True)

df_norm_median_all.to_csv(
    os.path.join(out_dir, "{0}_norm_median_all.csv".format(project_name)), index=False
)
df_read_ct_all.to_csv(
    os.path.join(out_dir, "{0}_read_ct.csv".format(project_name)), index=False
)
df_gap_all.to_csv(
    os.path.join(out_dir, "{0}_gap_summary.csv".format(project_name)), index=False
)
