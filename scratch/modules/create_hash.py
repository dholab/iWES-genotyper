...
def compare_full(df_exp_c, df_e, file_count=10):
    ...
    for idx, chunk in enumerate(np.array_split(unique_allele_list, file_count)):
        ...
        allele_list.pop(-1)
        df_i["NEW_GROUP"] = [
            1 if ((x != y) or (z != w)) else 0
            for x, y, z, w in zip(
                df_i["ALLELE_2"], allele_2_list, df_i["ALLELE"], allele_list
            )
        ]
        for col_i in ["START", "START_2", "END", "END_2"]:
            ...
            df_i["{0}_DIFF".format(col_i)] = [
                np.nan if x > 0 else y
                for x, y in zip(df_i["NEW_GROUP"], df_i["{0}_DIFF".format(col_i)])
            ]

        df_i["SUM_POS"] = df_i["START"] + df_i["END"]
        print("Find max")
        df_i["MAX_POS"] = df_i.groupby(["ALLELE", "ALLELE_2"])["SUM_POS"].transform(
            "max"
        )
        print(df_i)
        df_m = df_i[
            df_i["START_DIFF"].isna()
            | (df_i["START_DIFF"] > 1)
            | (df_i["END_DIFF"] > 1)
            | (df_i["MAX_POS"] == df_i["SUM_POS"])
        ]
        df_m["KEEP_LAST"] = [
            1 if ((x == y) and (not (z < 2))) else 0
            for x, y, z in zip(df_m["SUM_POS"], df_m["MAX_POS"], df_m["END_DIFF"])
        ]
        df_m["END_SHIFT"] = [
            x if y < 2 else x - y for x, y in zip(df_m["END"], df_m["END_DIFF"])
        ]
        df_m["LAST"] = [
            1 if (x == y) else 0 for x, y in zip(df_m["SUM_POS"], df_m["MAX_POS"])
        ]
        df_m["END_2_SHIFT"] = [
            x if y < 2 else x - z
            for x, y, z in zip(df_m["END_2"], df_m["END_DIFF"], df_m["END_2_DIFF"])
        ]
        ...
        df_n = df_n[
            ["ALLELE", "START", "END_SHIFTED", "ALLELE_2", "START_2", "END_2_SHIFTED"]
        ]
        df_n.rename(
            columns={"END_SHIFTED": "END", "END_2_SHIFTED": "END_2"}, inplace=True
        )
...
df = fasta_to_df(
    fasta_path=bait_fasta, header_name="ALLELE", sequence_name="SEQUENCE", as_df=True
)
...
df_exp = df_exp[
    ["ALLELE", "REF_SEQ_LEN", "SEQ_LIST", "START", "END", "LEN"]
