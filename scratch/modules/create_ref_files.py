...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--ref_dir", type=str, help="where the config files are stored", required=True
    )
    parser.add_argument(
        "-d",
        "--db_path",
...
def write_sequence(
    name, seq, out_file, replace_illegal_char=True, prefix="", suffix=""
):
...
def rev_comp_like_join(inner_seq, df_ref):
    ...
    for idx, row in df_ref.iterrows():
        if (inner_seq in row["SEQUENCE"]) or (
            rev_comp_st(inner_seq) in row["SEQUENCE"]
        ):
...
for i in range(1, len(db_ext)):
    ...
    concat_fasta_path = os.path.join(
        ref_dir, "{0}_concat.fasta".format(concat_suffix_str)
    )
    with open(concat_fasta_path, "w") as out_file:
        for suffix in concat_suffix_list:
            fasta_sequences = SeqIO.parse(
                open(os.path.join(ref_dir, "{0}.fasta".format(suffix))), "fasta"
            )
            for fasta in fasta_sequences:
                name = fasta.name
                write_sequence(
    ...
    if len(trimmed_sequence) > 0:
        read_list_mapped, trimmed_sequence = zip(
            *sorted(zip(read_list_mapped, trimmed_sequence))
        )
        print(trimmed_sequence[:3])
        print(read_list_mapped[:3])

        with open(
            os.path.join(ref_dir, "{0}_ungrouped.fasta".format(db_ext[i])), "w"
        ) as out_file:
        ...
        df_diag = fasta_to_df(
            fasta_path=os.path.join(ref_dir, "{0}_ungrouped.fasta".format(db_ext[i])),
            header_name="allele",
            sequence_name="SEQUENCE",
            as_df=True,
        )
        # remove any duplicate sequences as we have them grouped already
        df_diag_dedup = df_diag.drop_duplicates(
            subset=["SEQUENCE"], keep="first", ignore_index=False
        )
        ...
        df_diag_merge = (
            df_diag_merge.groupby(["SEQUENCE", "allele_first"])
        ...
        df_diag_merge["allele_group_name"] = [
            x.split("-")[1] if x.count("-") > 1 else x.split("-")[0]
            for x in df_diag_merge["allele_first"]
        ]
        df_diag_merge["allele_group_name"] = [
            "_".join(x.split("_")[:3]) for x in df_diag_merge["allele_group_name"]
        ]
        # df_diag_merge['allele_group_name'] = [x.replace('__', '_') for x in df_diag_merge['allele_group_name']]
        df_diag_merge["group_number"] = (
            df_diag_merge.groupby(["allele_group_name"])["allele_group_name"]
            .cumcount()
            .add(1)
        )
        df_diag_merge["header"] = [
            "{0}-{1}g{2}|{3}".format(species, x, y, z) if z.count(",") > 0 else j
            for x, y, z, j in zip(
                df_diag_merge["allele_group_name"],
                df_diag_merge["group_number"],
                df_diag_merge["allele"],
    ...
    df_diag_2 = fasta_to_df(
        fasta_path=os.path.join(ref_dir, "{0}.fasta".format(db_ext[i])),
        header_name="header",
        sequence_name="SEQUENCE",
        as_df=True,
    )
    ...
    with open(
        os.path.join(ref_dir, "{0}_long.fasta".format(db_ext[i])), "w"
    ) as out_file:
    ...
    df_diag_fasta["header"] = [
        "{0}-{1}".format(x.split("|")[0], db_ext[i]) if x.count("|") > 0 else x
        for x in df_diag_fasta["header"]
    ]
    df_diag_fasta["header"] = [
        x if x.endswith("-{}".format(db_ext[i])) else "{0}-{1}".format(x, db_ext[i])
        for x in df_diag_fasta["header"]
    ]
    with open(
        os.path.join(ref_dir, "{0}_all.fasta".format(db_ext[i])), "w"
    ) as out_file:
        for indx, row in df_diag_fasta.iterrows():
            if len(row["SEQUENCE"]) > 130:
                write_sequence(
                    name=row["header"].split("|")[0],
                    seq=row["SEQUENCE"],
                    out_file=out_file,
                    replace_illegal_char=True,
                    prefix="",
...
for i in range(1, len(db_ext)):
    ...
    for suffix in suffix_list:
        ...
        with open(
            os.path.join(ref_dir, f"{suffix}_to_{compare_suffix}_lookup.json"), "w"
        ) as convert_file:
        ...
        df_missing_ipd.to_csv(
            os.path.join(ref_dir, f"{suffix}_to_{compare_suffix}_missing.csv"),
            header=False,
            index=False,
        )
        ...
        # out put to gson
        with open(
            os.path.join(ref_dir, f"{compare_suffix}_to_{suffix}_lookup.json"), "w"
        ) as convert_file:
        ...
        df_missing_diag.to_csv(
            os.path.join(ref_dir, f"{compare_suffix}_to_{suffix}_missing.csv"),
            header=False,
            index=False,
        )
...
if (haplo_fasta is None) or (not os.path.exists(haplo_fasta)):
    print(
        "The haplo type summary files are not specified or do not exist.  They will not be created."
    )
    exit(0)
###############################
...
df_diag_haplo_ref = fasta_to_df(
    haplo_fasta, header_name="allele", sequence_name="SEQUENCE", as_df=True
)
# merge with the data frame by name with the haplotype calling dictionary
df_melt["haplo_allele"] = [
    like_join_haplo(x, df_diag_haplo_ref, "allele", "allele") for x in df_melt["allele"]
]
...
print(df_bait_ref)
df_haplo_call_m["bait_allele"] = [
    like_join_haplo(x, df_bait_ref, "SEQUENCE", "allele")
    for x in df_haplo_call_m["SEQUENCE"]
]
