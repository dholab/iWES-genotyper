...
from openpyxl.styles import Fill, Font, GradientFill, PatternFill
...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument("--depth_threshold", type=int, default=3, required=False)
    parser.add_argument(
        "-e",
        "--db_ext",
        nargs="+",
    ...
    parser.add_argument("--max_gap", default=120, type=int, required=False)
...
for i in range(1, len(db_ext)):
    ...
    for db_c_i in suffix_list:
        ...
        lookup_json_path = os.path.join(
            ref_dir, "{0}_to_{1}_lookup.json".format(db_i, db_c_i)
        )
    ...
    if os.path.exists(os.path.join(ref_dir, "{0}_lu.csv".format(db_ext[i]))):
        ...
        for index, row in df_exon_lu.iterrows():
            if row["allele"] in ambig_dict_temp.keys():
                ambig_dict_temp[row["allele"]] = ambig_dict_temp[row["allele"]] + [
                    row["allele_grouped"].split("|")[1]
                ]
            else:
...
df_norm_median = pd.read_csv(
    os.path.join(out_dir, "{0}_norm_median_all.csv".format(project_name))
)
# The sample is often a number, let coerce it to be a string.
df_norm_median["SAMPLE_NUM"] = df_norm_median["SAMPLE_NUM"].astype(str)
df_norm_median.rename(
    columns={"ALLELE": "allele", "SAMPLE_NUM": "accession", "DEPTH_ADJ": "read_ct"},
    inplace=True,
)
...
for i in range(1, len(db_ext)):
    ...
    if len(df_lu_dict[db_i]) > 0:
        ...
        df_m_i = df_m_i[
            (df_m_i["Depth_Threshold_From_End_Position"] == 0)
            & (df_m_i["Depth_Threshold_From_Start_Position"] == 0)
        ]
        df_m_i = df_m_i.merge(
            df_lu_dict[db_i], on=["{0}_allele".format(db_i)], how="left"
        )
        df_m_i["allele"] = df_m_i["allele"].astype(str)
        df_norm_median_i = df_norm_median[df_norm_median["DB"].isin(suffix_list)]
        df_norm_median_i = df_norm_median_i[
            ["allele", "accession", "unique_maps_per_allele"]
        ]
        ...
        df_m_i = (
            df_m_i.groupby(
                [
                    "{0}_allele".format(db_i),
                    "read_ct",
                    "accession",
                    "unique_maps_per_allele_{0}".format(db_i),
                ]
            )["unique_maps_per_allele"]
            .max()
            .reset_index()
            .rename(columns={"{0}_allele".format(db_i): "allele"})
        )
        ...
        df_m_i.rename(
            columns={"unique_maps_per_allele_miseq": "unique_maps_per_allele"},
            inplace=True,
        )
        ...
        df = pd.concat(
            [df, df_norm_median[df_norm_median["DB"] == db_i]], ignore_index=True
        )
...
##############################################################
df_read_ct_pivot = df_read_ct.pivot_table(
    values="sample_read_ct", columns=["gs_id"], aggfunc=np.max, fill_value=0
).reset_index()
...
##############################################################################
df_median_counts = (
    df.groupby(["accession"])["read_ct"]
    .median()
    .reset_index()
    .rename(columns={"read_ct": "normalized_median_allele_count"})
)
df_median_counts["normalized_median_allele_count"] = df_median_counts[
    "normalized_median_allele_count"
].round(0)
...
if len(confidence_coeff_dict):
    df_median_counts["P_VALUE"] = [
        get_confidence(x, conf_group, confidence_coeff_dict, depth_threshold)
        for x in df_median_counts["normalized_median_allele_count"]
    ]
else:
    df_median_counts["P_VALUE"] = ""
df_median_counts_pivot = df_median_counts.pivot_table(
    values=["normalized_median_allele_count", "P_VALUE"],
    columns=["accession"],
    aggfunc=np.max,
    fill_value=0,
).reset_index()
...
df_allele_count_summary.rename(
    columns={"read_ct": "# Alleles Identified"}, inplace=True
)
df_count_summary_pivot = df_allele_count_summary.pivot_table(
    values="# Alleles Identified", columns=["gs_id"], aggfunc=np.max, fill_value=0
).reset_index()
...
genotype_pivot["gs_id"] = [
    "-".join(x.split("-")[1:]) if x.count("-") > 1 else x
    for x in genotype_pivot["gs_id"]
]

genotype_pivot["ambiguous_alleles"] = [
    x if x not in ambig_dict.keys() else ambig_dict[x] for x in genotype_pivot["gs_id"]
]
...
genotype_pivot["gs_id"] = genotype_pivot["gs_id"].str.replace(
    "A1:028g1-miseq", "AG3:02g1_A028-miseq"
)
genotype_pivot["gs_id"] = genotype_pivot["gs_id"].str.replace(
    "A1:110g1-miseq", "A1:110g1-cdna-miseq"
)
genotype_pivot["gs_id"] = genotype_pivot["gs_id"].str.replace(
    "A1:119g1-miseq", "AG3:02g2_A119-miseq"
)
...
df_list = [
    df_gs_id,
    df_count_summary_pivot,
    df_read_ct_pivot,
    df_median_counts_pivot,
    genotype_pivot,
]
...
if os.path.exists(haplotype_lookup):
    ...
    df_haplo_call.rename(
        columns={"allele": "allele_short", "bait_allele": "allele"},
        inplace=True,
        errors="ignore",
    )
    # merge the lookup to each call
    df_haplo_merge = df_haplo_call.merge(
        df_haplo_ready[["allele", "gs_id"]], on=["allele"], how="inner"
    )
    df_haplo_merge = df_haplo_merge[
        ["HAPLOTYPE_CALL", "PREFIX", "TYPE", "allele", "gs_id", "CALL_COUNT"]
    ].drop_duplicates()
    # find the counts of each call to figure out if all the allele types are covered
    df_haplo_merge["CALL_GSID_COUNT"] = df_haplo_merge.groupby(
        ["HAPLOTYPE_CALL", "PREFIX", "TYPE", "gs_id"]
    ...
    df_haplo_pass = df_haplo_merge[
        df_haplo_merge["CALL_COUNT"] == df_haplo_merge["CALL_GSID_COUNT"]
    ]

    # drop the duplicates and remove the allele names, we just care about the haplotype call
    df_haplo_pass = df_haplo_pass[
        ["HAPLOTYPE_CALL", "PREFIX", "TYPE", "gs_id", "CALL_COUNT"]
    ].drop_duplicates()
    # figure out how many haplotypes are call per group (1: Homozygous, 2: Heterozygous, >2 = Too many Haplotypes)
    df_haplo_pass["PASS_COUNT"] = df_haplo_pass.groupby(["gs_id", "TYPE"])[
        "HAPLOTYPE_CALL"
    ].transform("count")
    # rank them so we can retain the proper ones in the right order.
    df_haplo_pass["rank"] = df_haplo_pass.groupby(["gs_id", "TYPE"])[
        "CALL_COUNT"
    ].transform("rank", method="first")
    # join to create a list for the TMH
    df_haplo_pass["TMH"] = df_haplo_pass.groupby(["gs_id", "TYPE"])[
        "HAPLOTYPE_CALL"
    ].transform("; ".join)
    # Now use t he error code to report TMH or Heterozygous.
    df_haplo_pass["HAPLOTYPE_FINAL"] = [
        error_code(x, y, z)
        for x, y, z in zip(
            df_haplo_pass["HAPLOTYPE_CALL"],
            df_haplo_pass["PASS_COUNT"],
            df_haplo_pass["TMH"],
        )
    ]
    ...
    df_haplo_pass["TYPE"] = [
        "{0} {1}".format(x.replace("_", " "), int(y))
        for x, y in zip(df_haplo_pass["TYPE"], df_haplo_pass["rank"])
    ]
    ...
    df_haplo_pivot = df_haplo_pass.pivot_table(
        values="HAPLOTYPE_FINAL",
        index=["TYPE"],
        columns=["gs_id"],
        aggfunc=np.max,
        fill_value="NO HAPLO",
...
if (len(animal_lookup_path) > 0) and os.path.exists(animal_lookup_path):
    ...
    animal_list2.sort()
    col_order = (
        ["Animal IDs", "# Obs"] + animal_list2 + ["header_name", "ambiguous_alleles"]
    )
    ...
    col_order = (
        ["Animal IDs", "# Obs"] + col_names + ["header_name", "ambiguous_alleles"]
    )
    ...
    if row["allele"] in allelle_list:
        ...
        if row["unique_maps_per_allele"] == 1:
            ...
            if not row["allele"].endswith("-{0}".format(db_ext[0])):
                ws["{0}{1}".format(col_letter_i, row_i)].font = Font(
                    color="006400", bold=True
                )
            if row["DEPTH_NORM"] < 0.75:
                # attribute_list.append('background-color: #F2DFA9')
                ws["{0}{1}".format(col_letter_i, row_i)].fill = PatternFill(
                    start_color="F2DFA9", fill_type="solid"
                )
        else:
            if row["allele"].endswith("-{0}".format(db_ext[0])):
                if row["DEPTH_NORM"] < 0.75:
                    # attribute_list.append('background-color: #F2DFA9')
                    ws["{0}{1}".format(col_letter_i, row_i)].fill = GradientFill(
                        stop=("F2DFA9", "F2CAFA")
                    )
                else:
                    ws["{0}{1}".format(col_letter_i, row_i)].fill = PatternFill(
                        start_color="F2CAFA", fill_type="solid"
                    )
        ...
        if row["Depth_Threshold_From_Start_Position"] > 0:
            comment_i_list.append(
                "Depth_Threshold_From_Start_Position: {0}".format(
                    row["Depth_Threshold_From_Start_Position"]
                )
            )
        if row["Depth_Threshold_From_End_Position"] > 0:
            comment_i_list.append(
                "Depth_Threshold_From_End_Position: {0}".format(
                    row["Depth_Threshold_From_End_Position"]
                )
            )
        if row["MAX_GAP"] > 70:
            comment_i_list.append(
                "MAX_GAP: {0}, Gap_Position: {1}".format(
                    row["MAX_GAP"], row["GAP_POSITION"]
                )
            )
