...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--cp_dir", type=str, help="directory your bbmap executables are", required=True
    )
...
fastq_filelist = [
    os.path.join(fastq_dir, x)
    for x in fastq_filelist
    if x.endswith("R1_001.fastq.gz") and not x.startswith("._")
]
...
def print_time(line, update_reads, total_time, samples_procesed):
    try:
        ...
        pass
    print(
        "{0} seconds to process last {1}M reads. {2:.1f} total time / {3:.1f}M total reads".format(
            line.strip(),
...
for in_1, in_2, sample_i in zip(fastq_filelist, fastq2_filelist, sample_list):
    ...
    samples_procesed = 0
    with Popen(
        cmd_list, stdout=PIPE, stderr=PIPE, bufsize=1, universal_newlines=True
    ) as p1:
