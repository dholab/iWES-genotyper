...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "-d", "--db_suffix", help='suffix of the db i.e. "gen"', required=False
    )
    parser.add_argument(
        "--result_dir",
        type=str,
...
for out_dir in out_dirs:
    ...
    file_list = [
        ...
        for x in file_list
        if not x.startswith("._") and (f".depth_all-{db_suffix}.csv" in x)
    ]
    sample_list = []
    ...
    for file_i in file_list:
        ...
        df_i["ALLELE_CLASS"] = [
            x if x != "NKG2C-3-gen" else "NKG2C3-gen" for x in df_i["ALLELE_CLASS"]
        ]
...
for class_i in class_list:
    ...
    df[df["ALLELE_CLASS"] == class_i].to_csv(
        os.path.join(result_dir, f"{prefix}{class_i}_depth.csv.gz"), index=False
    )
...
for allele_i in allele_list:
    ...
    df_max_i = pd.concat(
        [
            pd.DataFrame({"ALLELE": [allele_i], "POSITION": 0, "DEPTH": threshold + 1}),
            df_max_i,
            pd.DataFrame(
                {"ALLELE": [allele_i], "POSITION": pos_max_1, "DEPTH": threshold + 1}
            ),
        ],
        ignore_index=True,
    )
...
with open(os.path.join(ref_dir, f"{prefix}nmask.fasta"), "w") as f:
    ...
    for allele_i, seq_i in fasta_seq_dict.items():
        ...
        if len(end_list) > 0:
            seq_n = [
                "N" if test_ranges(y, start_list, end_list) else x
                for x, y in zip(seq_i, range(1, len(seq_i) + 1))
            ]
            seq_str = "".join(seq_n)
        else:
            seq_str = seq_i
