...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--in_dir", type=str, help="directory to output the results", required=True
    )
    parser.add_argument(
        "--submit_name", type=str, help="Project name for file prefix", required=True
    )
...
file_list = [
    x for x in file_list if not x.startswith("._") and not x.startswith(submit_name)
]
...
for median_i in median_all_list:
    try:
        ...
        df_norm_median = pd.concat(
            [df_norm_median, df_norm_median_i], ignore_index=True
        )
...
for gaps_i in gaps_list:
    try:
        ...
        continue
df_norm_median.to_csv(
    os.path.join(in_dir, "{0}_norm_median_all.csv".format(submit_name)), index=False
)
df_read_ct.to_csv(
    os.path.join(in_dir, "{0}_read_ct.csv".format(submit_name)), index=False
)
