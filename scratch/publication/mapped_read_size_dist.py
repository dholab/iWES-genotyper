...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--out_dir", type=str, help="path to ypur depth files.", required=True
    )
...
for filepath_i in filelist:
    ...
    df = df[["NAME", "START", "END", "REVERSED_MAP", "REF_LEN"]].drop_duplicates(
        subset=["NAME", "REVERSED_MAP"]
    )
    df_m = df.melt(
        id_vars=["NAME", "REVERSED_MAP", "REF_LEN"],
    ...
    df_pivot["MAPPED_LENGTH"] = [
        frag_len(w, x, y, z)
        for w, x, y, z in zip(
            df_pivot["data|for|END"],
    ...
    df_pivot_i = df_pivot_i[
        (df_pivot_i["data|for|START"] != 0) & (df_pivot_i["data|rev|START"] != 0)
    ]
    df_pivot_i = df_pivot_i[
        (df_pivot_i["data|for|END"] != df_pivot_i["REF_LEN"])
        & (df_pivot_i["data|rev|END"] != df_pivot_i["REF_LEN"])
    ]
