...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--depth_dir", type=str, help="path to ypur depth files.", required=True
    )
    parser.add_argument(
        "--out_dir",
        type=str,
    ...
    parser.add_argument("--overwrite", type=str2bool, default=False, required=False)
    parser.add_argument(
        "-c",
        "--mhc_class",
        nargs="+",
...
def get_aligned_positions_df(
    df_fasta, seq_comp_id, comparing_strain_prefix="COMPARING", peptide_length=1
):
    import math
...
def open_fasta_as_df(fasta_path):
    ...
    return pd.DataFrame(
        {"SEQ_ID": seq_id, "MHC_CLASS": mhc_class, "SEQUENCE": seq, "DB": db}
    )
...
if not os.path.exists(depth_all_path) or overwrite:
    ...
    file_list = [
        os.path.join(depth_dir, x)
        for x in file_list
        if not x.startswith("._") and x.endswith("depth-gen.csv")
    ]
...
for mhc_class_i, db_i in itertools.product(mhc_class_list, db_ext_list):
    ...
    df_fasta_i = df_fasta[
        (df_fasta["MHC_CLASS"] == mhc_class_i) & (df_fasta["DB"] == db_i)
    ]
    header_list = list(df_fasta_i["SEQ_ID"].unique())
    out_path = os.path.join(
        ref_out_dir, "{0}__{1}_aligned.fasta".format(mhc_class_i, db_i)
    )
    in_path = os.path.join(
        ref_out_dir, "{0}__{1}_filtered.fasta".format(mhc_class_i, db_i)
    )
    ...
    if not os.path.exists(out_path) or overwrite:
        ...
        if use_docker:
            ...
            subprocess.call(
                [muscle_path, "-in", in_path, "-out", out_path], shell=False
            )
    aligned_csv_path = os.path.join(
        out_dir, f"{mhc_class_i}__{db_i}_aligned_fasta.csv.gz"
    )
    if not os.path.exists(aligned_csv_path) or overwrite:
        ...
        df_aligned = get_aligned_positions_df(
            df_fasta=df_aligned_fasta,
    ...
    df_aligned_m = df_aligned.merge(df_depth, on=["POSITION", "ALLELE"], how="inner")
    aligned_depth_path = os.path.join(
        out_dir, f"aligned_depth_{mhc_class_i}__{db_i}.csv.gz"
    )
    print(
        f"Saving aligened depth file, not aggregated by compared position: {aligned_depth_path}"
    )
    ...
    df_aligned_m_agg = df_aligned_m.groupby(
        ["COMPARING_POSITION", "ALLELE", "gs_id"], as_index=False
    )[agg_mean].mean()
    df_aligned_m_agg[agg_mean] = df_aligned_m_agg[agg_mean].round(2)
    aligned_depth_agg_path = os.path.join(
        out_dir, f"aligned_depth_{mhc_class_i}__{db_i}_aggregated.csv"
    )
    print(
        f"Saving aligned depth file: aggregating aligned csv to aggregated depth files to: {aligned_depth_agg_path}"
    )
