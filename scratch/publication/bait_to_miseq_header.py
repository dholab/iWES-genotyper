...
df_miseq_ref = fasta_to_df(
    miseq_fasta, header_name="allele", sequence_name="SEQUENCE", as_df=True
)
df_bait_ref = fasta_to_df(
    bait_fasta,
    header_name="allele",
...
for df_i in df_chunk_array:
    print(counter)
    df_i[f"{db_type}_allele"] = [
        like_join_haplo2(x, df_miseq_ref, "SEQUENCE", "allele")
        for x in df_i["SEQUENCE"]
    ]
