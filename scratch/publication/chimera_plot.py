...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--out_dir", type=str, help="path to your output dir.", required=True
    )
    parser.add_argument(
        "--prefix",
        type=str,
...
filelist = [
    ...
    for x in filelist
    if not x.startswith("._") and x.endswith("_expanded_maps-gen.csv")
]
...
for file_i in filelist:
    ...
    df_i["MAX_END"] = df_i.groupby(["TOKEN_NAME", "ALLELE", "ALLELE_2_y.1"])[
        "END"
    ].transform(max)
    df_i["MIN_START"] = df_i.groupby(["TOKEN_NAME", "ALLELE", "ALLELE_2_y.1"])[
        "START"
    ].transform(min)
    df_i["MAX_LEN"] = df_i["MAX_END"] - df_i["MIN_START"]
    df_i["SEGMENT_LEN"] = [
        y if y < 286 else x for x, y in zip(df_i["LENGTH"], df_i["MAX_LEN"])
    ]

    # df_i = df_i[~((df_i['REVERSED_MAP']=='rev')& (df_i['SEGMENT_LEN']>=df_i['LENGTH']))]
    df_i["TOKEN_NAME_DIR"] = [
        f"{x}_{y}" for x, y in zip(df_i["TOKEN_NAME"], df_i["REVERSED_MAP"])
    ]

    df_i = df_i[df_i["ALLELE"].isin(allele_list)]
    df_i = df_i[(df_i["START"] > start) & (df_i["START"] < end)]
    df_i = df_i.drop_duplicates(
        subset=["TOKEN_NAME_DIR", "ALLELE", "REVERSED_MAP", "ALLELE_2_y.1"]
    )

    df_i["ALLELE_MAPPING_LIST"] = df_i.groupby(
        ["TOKEN_NAME", "ALLELE", "REVERSED_MAP"]
    )["ALLELE_2_y.1"].transform(lambda x: "\n".join(x))
    ...
    for allele_i, color_i in zip(
        allele_mapping_list, color_list[: len(allele_mapping_list)]
    ):
        c_dict[allele_i] = color_i
    print(c_dict)
    df_i["color"] = df_i.apply(
        lambda x: color(x["ALLELE_MAPPING_LIST"], c_dict), axis=1
    )
